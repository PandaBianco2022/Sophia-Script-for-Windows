...
"@
	...
	if (-not (Get-AppxPackage -Name MicrosoftWindows.Client.CBS -Verbose))
	{
		Write-Information -MessageData "" -InformationAction Continue
...
function AdminApprovalMode
{
	...
	# Remove all policies in order to make changes visible in UI only if it's possible
	Remove-ItemProperty -Path HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System -Name FilterAdministratorToken -PropertyType DWord -Force -ErrorAction Ignore
